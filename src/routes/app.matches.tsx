  ...
  return (
    ...
    >
      {src ? (
        ...
        <span className="text-label font-semibold">{initials(m.other?.name, m.other?.handle)}</span>
      )}
    </div>
  );
}
      ...
      <div className="px-5 pb-28 space-y-6">
        ...
        {!initialLoading && error && (
          <div className="surface p-6 text-center">
            ...
            <p className="mt-3 font-semibold text-lead">couldn't load your matches</p>
            <p className="mt-1 text-label text-muted-foreground">check your connection and try again.</p>
            <button
              onClick={() => refresh()}
              className="mt-4 inline-flex px-4 py-2 min-h-10 rounded-lg text-label font-semibold tap-scale"
        ...
        {!initialLoading && !error && active.length === 0 && expiredUnstarted.length === 0 && (
          <div className="surface p-8 text-center">
            ...
            <p className="mt-3 font-semibold text-lead">no matches yet</p>
            <p className="mt-1 text-label text-muted-foreground">the moment two people pick each other, they land here.</p>
            <Link
              to="/app/add"
              className="mt-5 inline-flex items-center gap-1.5 px-5 py-2.5 min-h-11 rounded-lg text-body font-semibold tap-scale"
              style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}
            >
                ...
                <li key={m.id}>
                  <div className="surface px-3 py-2.5 flex items-center gap-3 opacity-70">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-body truncate">{m.other?.name ?? "match"}</p>
                      <p className="text-caption text-muted-foreground truncate inline-flex items-center gap-1">
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-micro font-semibold uppercase tracking-wider text-muted-foreground mb-2 px-1">
      {children}
    </p>
  );
}
